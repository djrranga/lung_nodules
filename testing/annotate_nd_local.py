...
def annotate(uid, voxel_coords, diameter, dataset_root):
    files = glob.glob(f'{dataset_root}/images/subset*/{uid}*.npz')
    if not files:
        print(f"Warning: no image .npz found for uid={uid} (looking in {dataset_root}/images/subset*/{uid}*.npz). Skipping.")
        return False
    filepath = files[0]
    # Ensure corresponding labels directory exists
    labels_dir = Path(filepath).parent.as_posix().replace(f'{dataset_root}/images', f'{dataset_root}/labels')
    ...
    annofile = filepath.replace('images', 'labels').replace('.npz', f'.{sliceno:03d}.txt')
...
def main():
    ...
    if idx is None:
        ...
        if slurm_idx is not None:
            ...
            idx = None  # signal to process all rows locally
    ...
    if idx is not None:
        ...
        ok = annotate(uid, vox, dia, args.dataset)
        print(f"Row {idx}: uid={uid} -> {'ok' if ok else 'skipped'}")
    else:
        # process all rows sequentially
        total = len(rows)
        succeeded = 0
        for i, (uid, vox, dia) in enumerate(rows):
            ok = annotate(uid, vox, dia, args.dataset)
            if ok:
                succeeded += 1
            if (i + 1) % 50 == 0 or (i + 1) == total:
                print(f"Processed {i+1}/{total} rows (succeeded {succeeded})")
        print(f"Done. {succeeded}/{total} annotations written.")
